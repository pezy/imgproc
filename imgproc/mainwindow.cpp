    ...
    ui(new Ui::MainWindow), m_recentFiles(new RecentFiles(this))
{
	...
	connect(m_recentFiles, &RecentFiles::activated, this, &MainWindow::onRecentFilesTriggered);
	connect(ui->action_Open, &QAction::triggered, this, &MainWindow::onFileOpenActionTriggered);
    connect(ui->action_Save, &QAction::triggered, this, &MainWindow::onFileSaveActionTriggered);
    connect(ui->actionSave_As, &QAction::triggered, this, &MainWindow::onFileSaveAsActionTriggered);
	{
		using namespace QtOcv;
		
		connect(ui->originalView, &ImageWidget::colorUnderMouseChanged, this, &MainWindow::onColorUnderMouseChanged);
		connect(ui->processView, &ImageWidget::colorUnderMouseChanged, this, &MainWindow::onColorUnderMouseChanged);
...
{
    QAction *act = ui->menuProcess->addAction(text, this, SLOT(onImageActionTriggered()));
    act->setProperty("id", id);
    m_processActions.insert(id, act);
}
